...
class RandomResizeCrop(torch.nn.Module):
    ...
    def get_params(virtual_crop_size, in_size, time_scale, freq_scale):
        ...
        i = np.random.randint(0, canvas_h - h) if canvas_h > h else 0
        j = np.random.randint(0, canvas_w - w) if canvas_w > w else 0
        return i, j, h, w
...
class RandomLinearFader(torch.nn.Module):
...
class MixupBYOLA(torch.nn.Module):
    """Mixup for BYOL-A.
...
class MixGaussianNoise(torch.nn.Module):
...
class PrecomputedNorm(torch.nn.Module):
